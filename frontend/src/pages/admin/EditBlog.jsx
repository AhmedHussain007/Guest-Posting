...
import { useUpdateBlogMutation, useGetBlogsQuery } from "@/apis/blogApi";
...
const blogSchema = z.object({
  ...
  mainImage: z.union([z.instanceof(File), z.string().min(1)]),
...
const BlogWriterPage = () => {
  ...
  const blogToEdit = blogs?.find((blog) => blog.slug === slug);
  ...
  useEffect(() => {
    if (blogToEdit) {
      form.reset({
        ...
        mainImage: blogToEdit.mainImage, // Keep string URL for preview
        sections: blogToEdit.sections,
      });
    }
  }, [blogToEdit, form]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await updateBlog({ blog: slug, data: formData }).unwrap();
    ...
    <div className="bg-black">
      ...
      <div className="h-screen py-16 bg-black flex items-center justify-center">
        <div className="w-full bg-black h-full flex p-4 px-12 gap-8">
          {/* LEFT - Preview */}
          <div className="hidden lg:flex w-2/3 flex-col p-6 bg-black rounded-lg overflow-y-auto border border-gray-700">
            <h2 className="text-lg font-bold mb-4 text-white">Live Preview</h2>
            <LeftContent blog={previewBlog} preview={true} />
          </div>

          {/* RIGHT - Form */}
          <div className="max-w-md m-auto w-full lg:md:w-1/3 flex flex-col h-[90vh] items-center overflow-y-auto text-white">
            <p className="mt-4 text-xl font-bold">Write a New Blog</p>
            <div className="my-7 flex items-center justify-center text-gray-300">
